...
export const listPluginsEndpoint =
 ...
 */
export const getBase64Credentials = (
  accountSid: string,
  authToken: string
): string => {
...
export const getAllPlugins = async (context: ApplicationContext) => {
  ...
  const plugins = await axios
    .get("https://flex-api.twilio.com/v1/PluginService/Plugins?PageSize=20", {
      headers: {
...
export const getPluginVersion = async (
  ...
  const version = await axios
    .get(
      `https://flex-api.twilio.com/v1/PluginService/Plugins/${pluginSid}/Versions/${versionSid}`,
      {
        headers: {
          Authorization: `Basic ${base64Credentials}`,
        },
 ...
 * @returns versions
 */
export const getAllPluginVersions = async (
  context: ApplicationContext,
  pluginSid: string
) => {
  const accountSid = await getParam(context, ACCOUNT_SID);
  ...
  const versions = await axios
    .get(
      `https://flex-api.twilio.com/v1/PluginService/Plugins/${pluginSid}/Versions?PageSize=20`,
      {
        headers: {
          Authorization: `Basic ${base64Credentials}`,
        },
 ...
 */
export const createPluginConfiguration = async (
  context: ApplicationContext,
 ...
 */
export const getAllPluginConfigurations = async (
  context: ApplicationContext
): Promise<Configuration> => {
  const accountSid = await getParam(context, ACCOUNT_SID);
  const authToken = await getParam(context, AUTH_TOKEN);
  const base64Credentials = convertToBase64(accountSid + ":" + authToken);
  const configurations = await axios
    .get(
      "https://flex-api.twilio.com/v1/PluginService/Configurations?PageSize=20",
      {
        headers: {
          Authorization: `Basic ${base64Credentials}`,
        },
...
export const createPluginRelease = async (
  context: ApplicationContext,
...
export const nonLocalHostResponse = (response: TwilioResponse) => {
  response
    .setStatusCode(500)
    .setBody("You can only run this on localhost only!");
  return response;
};
